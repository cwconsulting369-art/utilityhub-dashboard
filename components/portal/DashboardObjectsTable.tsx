      ...
      >
        ...
        <motion.tbody variants={tableContainer} initial="hidden" animate="show">
          {objects.map((row, idx) => {
            ...
            const zNr = zNrRaw
              ? (zNrRaw.split(":").pop() ?? zNrRaw).trim() || null
              : null
            const knr =
              (row.customer_identities ?? []).find(
                (i) =>
                  i.system !== "weg" &&
                  (i.system === "knr" || i.system === "teleson")
              )?.external_id ?? null
            const objektLabel = getStreet(row.full_name)
            const statusColor =
              CUSTOMER_STATUS_COLOR[row.status] ?? "var(--text-muted)"
            const statusLabel =
            ...
            return (
              ...
              >
                ...
                >
                  ...
                  >
                    {row.object_type === "weg" ? "WEG" : "Privat"}
                  </span>
                </td>
                ...
                >
                  ...
                  >
                    ...
                    <span style={{ fontSize: "10px", fontWeight: 600, color: ageColor }}>
                      {ageLabel}
                    </span>
                  </div>
                </td>
